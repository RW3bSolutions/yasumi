  ...
  <div class="row q-px-md q-py-xs" v-if="role === 'Owner'">
    <div class="col-12 col-md-6 q-py-sm self-center">
      <q-breadcrumbs gutter="sm">
        <q-breadcrumbs-el label="Therapists" class="text-weight-medium" />
      </q-breadcrumbs>
    </div>
        ...
        <q-td :props="props">
          <q-btn-dropdown color="dark" label="Action" size="sm">
            <q-list class="q-pa-xs">
              ...
              <q-item clickable v-close-popup class="q-pa-xs" @click="commission.id = props.value.id, commission.modal = true">
  ...
  <q-dialog v-model="modal" v-if="role === 'Owner'">
    <q-card style="width: 700px; max-width: 80vw;">
      <form @submit.prevent="ifEdit ? update() : store()">
        <q-card-section>
          <div class="text-h6">{{ ifEdit ? 'UPDATE THERAPIST' : 'NEW THERAPIST' }}</div>
        ...
        <q-card-section>
          ...
          <div class="q-mt-sm">
            <q-input color="dark" label="Commission *" square="" v-model="form.commission" :dense="true" autofocus="" />
            <div class="text-caption text-red" v-if="errors.form.commission && errors.show">
              {{ errors.form.commission[0] }}
            </div>
          </div>
        </q-card-section>
        <q-separator/>
...
export default defineComponent({
  ...
  methods: {
    ...
    index () {
      this.$api.get('branch/therapists', {
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('token')
        }
      })
    ...
    store () {
      this.submitted = true
      this.errors.show = false
      this.$api.post('branch/therapists', {
        name: this.form.name,
    ...
    update () {
      this.submitted = true
      this.errors.show = false
      this.$api.post('branch/therapists/' + this.object.id, {
        name: this.form.name,
        commission: this.form.commission,
    ...
    destroy (param) {
      this.preload = true
      this.$api.post('branch/therapists/' + param, {
        _method: 'DELETE'
      }, {
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('token')
