  ...
  <div class="row q-px-md q-py-xs">
    <div class="col-12 col-md-6 q-py-sm self-center">
      <q-breadcrumbs gutter="sm">
        <q-breadcrumbs-el label="Manage Branches" class="text-weight-medium" />
      </q-breadcrumbs>
    </div>
        ...
        <q-td :props="props">
          <q-btn-dropdown color="dark" label="Action" size="sm">
            <q-list class="q-pa-xs">
              <q-item clickable v-close-popup class="q-pa-xs" :to="'/branches/' + props.value.id">
  ...
  <q-dialog v-model="modal">
    <q-card style="width: 700px; max-width: 80vw;">
      <form @submit.prevent="ifEdit ? update() : store()">
        <q-card-section>
          <div class="text-h6">{{ ifEdit ? 'UPDATE BRANCH' : 'NEW BRANCH' }}</div>
        ...
        <q-card-section>
          ...
          <div class="q-mt-sm">
            <q-input color="dark" label="Address *" square="" v-model="form.address" :dense="true" autofocus="" />
...
export default defineComponent({
  ...
  methods: {
    ...
    update () {
      this.submitted = true
      this.errors.show = false
      this.$api.post('admin/branches/' + this.object.id, {
        name: this.form.name,
        address: this.form.address,
        is_active: this.form.is_active,
        _method: 'PUT'
        ...
        .then(response => {
          ...
          this.object.address = this.form.address
    ...
    destroy (param) {
      this.preload = true
      this.$api.post('admin/branches/' + param, {
        _method: 'DELETE'
      }, {
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('token')
