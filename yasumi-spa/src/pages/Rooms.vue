  ...
  <div class="row q-px-md q-py-xs" v-if="role === 'Owner'">
    <div class="col-12 col-md-6 q-py-sm self-center">
      <q-breadcrumbs gutter="sm">
        <q-breadcrumbs-el label="Rooms" class="text-weight-medium" />
      </q-breadcrumbs>
    </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    update () {
      this.submitted = true
      this.errors.show = false
      this.$api.post('branch/rooms/' + this.object.id, {
        name: this.form.name,
        commission: this.form.commission,
    ...
    destroy (param) {
      this.preload = true
      this.$api.post('branch/rooms/' + param, {
        _method: 'DELETE'
      }, {
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('token')
