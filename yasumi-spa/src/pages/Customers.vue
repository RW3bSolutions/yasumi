  ...
  <div class="row q-px-md q-py-xs">
    <div class="col-12 col-md-6 q-py-sm self-center">
      <q-breadcrumbs gutter="sm">
        <q-breadcrumbs-el label="Customers" class="text-weight-medium" />
      </q-breadcrumbs>
    </div>
  ...
  <q-dialog v-model="modal">
    <q-card style="width: 700px; max-width: 80vw;">
      <form @submit.prevent="ifEdit ? update() : store()">
        <q-card-section>
          <div class="text-h6">{{ ifEdit ? 'UPDATE CUSTOMER' : 'NEW CUSTOMER' }}</div>
...
export default defineComponent({
  ...
  methods: {
    ...
    destroy (param) {
      this.preload = true
      this.$api.post('branch/customers/' + param, {
        _method: 'DELETE'
      }, {
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('token')
